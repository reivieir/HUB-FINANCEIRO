...
const App: React.FC = () => {
  ...
  // --- 2. ESTADOS DE CHAT IA (LÓGICA ORIGINAL RESTAURADA) ---
  ...
  const selectItem = (origin: Origin | 'ranking' | 'ppt', index: number) => {
    if (origin === 'faq') {
      ...
      // Lógica de formulários para comandos específicos
      if (item.t === "Recuperar Acesso" || item.t === "Alterar Email Cadastrado") {
        setModalTitle(item.t === "Recuperar Acesso" ? "DADOS USUÁRIO" : "ALTERAÇÃO E-MAIL");
  ...
  const handleAskAI = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'model', text: "A IA está processando. Aguarde um instante ou tente novamente." }]);
    ...
    <div className="min-h-screen bg-[#0B0C10] p-4 md:p-8 font-sans">
      ...
      {currentView === 'home' ? (
        <div className="max-w-6xl mx-auto flex flex-col gap-8 animate-fade-in italic">
          ...
          <div className="bg-[#121418] rounded-[40px] shadow-2xl border border-gray-800 flex flex-col min-h-[420px] overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-[#0B0C10] custom-scrollbar">
              {messages.length === 0 && <p className="text-center text-gray-700 font-bold uppercase text-[9px] mt-20">Consulte a base de dados técnica aqui...</p>}
              {messages.map((msg, i) => (
          ...
          {/* Listas de Risco Sacado Visíveis (constants.ts) */}
        ...
        <div className="max-w-4xl mx-auto bg-[#121418] rounded-[40px] p-16 border border-gray-800 text-center animate-slide-up italic">
           ...
           <p className="text-gray-600 font-bold uppercase text-xs mt-4 mb-12 tracking-widest">Interface em Desenvolvimento...</p>
      ...
      {/* Modal de Inputs (Restaurado para os Comandos) */}
