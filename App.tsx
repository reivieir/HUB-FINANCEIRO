...
// Conexão com o seu arquivo original
...
const App: React.FC = () => {
  ...
  // Inicialização idêntica à que você validou no primeiro código
  useEffect(() => {
    if (isAuthenticated && !chatSession) {
      createDexcoChat()
        .then(session => setChatSession(session))
        .catch(err => console.error("Erro na carga da IA:", err));
    }
  ...
  // --- 4. FUNÇÃO DE IA (VOLTANDO AO MÉTODO ESTÁVEL) ---
  const handleAskAI = async (e: React.FormEvent) => {
    ...
    try {
      // Método sendMessage padrão para garantir o retorno
      ...
      setMessages(prev => [...prev, { role: 'model', text: "Conexão instável com a base DOCX. Tente novamente." }]);
    } finally {
    ...
    return (
      <div className="h-screen w-full flex items-center justify-center bg-[#0B0C10]">
        <form onSubmit={handleLogin} className="bg-white p-10 rounded-[32px] shadow-2xl border-t-[10px] border-[#D4A373] w-[380px] text-center italic">
          ...
          <button type="submit" className="w-full bg-black text-[#D4A373] py-4 rounded-2xl font-black uppercase text-xs">Acessar Sistema</button>
        </form>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#0B0C10] p-4 md:p-8 font-sans italic">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 italic">
        ...
        <div className="bg-[#121418]/40 p-6 rounded-3xl border-b-4 border-gray-800 opacity-30 cursor-default italic">
      ...
      {currentView === 'home' ? (
        <div className="max-w-6xl mx-auto flex flex-col gap-10 animate-fade-in italic">
          ...
          <div className="bg-[#121418] rounded-[40px] shadow-2xl border border-gray-800 flex flex-col min-h-[450px] overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-[#0B0C10] custom-scrollbar italic">
              {messages.length === 0 && <p className="text-center text-gray-700 font-bold uppercase text-[9px] mt-20 italic">HUB operando. Consulte a base técnica aqui...</p>}
              {messages.map((msg, i) => (
              ...
              {isAiLoading && <div className="text-[9px] font-black text-[#D4A373] animate-pulse uppercase italic">Processando...</div>}
              <div ref={chatEndRef} />
            </div>
            <form onSubmit={handleAskAI} className="p-6 bg-black border-t border-gray-800 flex gap-3 italic">
              ...
              <button type="submit" className="bg-[#D4A373] text-black px-10 rounded-2xl font-black uppercase text-xs italic">Enviar</button>
            </form>
          </div>

          {/* FAQ e Comandos (Restaurados na Home) */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 italic">
            <div className="bg-[#121418] rounded-[40px] p-8 border border-gray-800 shadow-xl italic">
               ...
               <div className="space-y-1 h-[400px] overflow-y-auto pr-2 custom-scrollbar italic">
                  {PERGUNTAS_FREQUENTES.map((f, i) => <ListItem key={i} text={f.p} onClick={() => selectItem('faq', PERGUNTAS_FREQUENTES.indexOf(f))} />)}
               </div>
            </div>
            <div className="bg-[#121418] rounded-[40px] p-8 border border-gray-800 shadow-xl italic">
               <h3 className="text-[11px] font-black text-green-500 uppercase mb-4 tracking-widest border-b border-gray-800 pb-2 italic">Comandos Cervello & Report</h3>
               <div className="space-y-1 h-[400px] overflow-y-auto pr-2 custom-scrollbar italic">
                  {COMANDOS_GEMS.map((c, i) => <ListItem key={i} text={c.t} onClick={() => selectItem('command', COMANDOS_GEMS.indexOf(c))} />)}
                  <div className="mt-4 pt-4 border-t border-gray-800 space-y-2 italic">
                     <ListItem text="Report Semanal Dexco" onClick={() => selectItem('ppt', 0)} />
                     <ListItem text="Ranking de Bancos" onClick={() => selectItem('ranking', 0)} />
                  </div>
               </div>
            </div>
          </div>
        ...
        <div className="max-w-4xl mx-auto bg-[#121418] rounded-[40px] p-16 border border-gray-800 text-center animate-slide-up italic">
           ...
           <p className="text-gray-600 font-bold uppercase text-xs mt-4 mb-10 tracking-widest italic">Módulo em integração de perfis...</p>
           <button onClick={() => setCurrentView('home')} className="bg-[#D4A373] text-black px-12 py-4 rounded-2xl font-black uppercase text-xs italic">Voltar ao HUB</button>
        </div>
      )}

      {/* POP-UP DE CÓPIA (JANELA FLUTUANTE) */}
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 italic">
           <div className="bg-white w-full max-w-2xl rounded-[32px] overflow-hidden shadow-2xl animate-scale-in italic">
              <div className="p-6 bg-black flex justify-between items-center italic">
                 ...
                 <button onClick={() => setSelected(null)} className="text-white font-black text-xl hover:text-red-500 italic">×</button>
              </div>
              <div className="p-8 italic">
                 ...
                 <div className="flex flex-col gap-3 italic">
                    {selected.body !== "VIEW_PPT_FRAME" && (
                      <button onClick={() => handleCopy()} className={`w-full py-5 rounded-2xl font-black uppercase text-sm transition-all ${isCopied ? 'bg-green-600 text-white shadow-lg' : 'bg-black text-[#D4A373] italic'}`}>
                         {isCopied ? 'COPIADO COM SUCESSO!' : 'COPIAR PARA O CERVELLO / E-MAIL'}
                      </button>
                    )}
      ...
      {/* Modal de Inputs para Formulários (Recuperar Acesso, etc) */}
