...
const App: React.FC = () => {
  ...
  // Inicialização idêntica ao seu primeiro código
  useEffect(() => {
    if (isAuthenticated) {
      createDexcoChat().then(session => setChatSession(session)).catch(e => console.error("Erro IA:", e));
    }
  }, [isAuthenticated]);
  ...
  return (
    <div className="min-h-screen bg-[#0B0C10] p-4 md:p-8 font-sans">
      ...
      <div className="max-w-6xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 italic">
        <button onClick={() => setCurrentView('home')} className={`p-6 rounded-3xl border-b-4 transition-all ${currentView === 'home' ? 'bg-[#D4A373] text-black' : 'bg-[#121418] border-[#D4A373] text-white'}`}>
           <p className="text-sm font-black uppercase">Risco Sacado</p>
        </button>
        <button onClick={() => setCurrentView('bancos')} className={`p-6 rounded-3xl border-b-4 transition-all ${currentView === 'bancos' ? 'bg-green-600 text-black' : 'bg-[#121418] border-green-600 text-white'}`}>
           <p className="text-sm font-black uppercase">Gestão Bancos</p>
        </button>
        <div className="bg-[#121418]/40 p-6 rounded-3xl border-b-4 border-gray-800 opacity-30 cursor-default"><p className="text-sm font-black text-gray-600 uppercase">Protestos (IA)</p></div>
        <div className="bg-[#121418]/40 p-6 rounded-3xl border-b-4 border-gray-800 opacity-30 cursor-default"><p className="text-sm font-black text-gray-600 uppercase">Conciliação (IA)</p></div>
      </div>
      ...
      {currentView === 'home' ? (
        <div className="max-w-6xl mx-auto flex flex-col gap-8 animate-fade-in italic">
          ...
          <div className="bg-[#121418] rounded-[40px] shadow-2xl border border-gray-800 flex flex-col min-h-[420px] overflow-hidden">
            <div className="p-4 bg-black text-center border-b border-[#D4A373]/20"><h2 className="text-[#D4A373] font-black uppercase text-[10px] tracking-[0.3em]">IA Financeira Ativa</h2></div>
            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-[#0B0C10] custom-scrollbar">
              {messages.length === 0 && <p className="text-center text-gray-600 font-bold uppercase text-[10px] mt-20">Consulte sobre Sacado, Bancos ou Protestos...</p>}
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[85%] p-5 rounded-3xl text-sm ${msg.role === 'model' ? 'bg-[#1A1D23] text-gray-200 border-l-4 border-[#D4A373]' : 'bg-[#D4A373] text-black font-bold'}`}>{msg.text}</div>
                </div>
              ))}
          ...
          {/* Conteúdo do constants.ts restaurado na Home */}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div className="bg-[#121418] rounded-[40px] p-8 border border-gray-800 shadow-xl">
               <h3 className="text-[11px] font-black text-[#D4A373] uppercase mb-6 tracking-widest italic">Dúvidas Frequentes</h3>
            ...
            <div className="bg-[#121418] rounded-[40px] p-8 border border-gray-800 shadow-xl">
               <h3 className="text-[11px] font-black text-green-500 uppercase mb-4 tracking-widest italic">Comandos Cervello & Report</h3>
               <div className="space-y-1 h-[350px] overflow-y-auto pr-2 custom-scrollbar">
        ...
        <div className="max-w-4xl mx-auto bg-[#121418] rounded-[40px] p-16 border border-gray-800 text-center italic animate-slide-up">
           ...
           <p className="text-gray-600 font-bold uppercase text-xs mt-4 mb-12">Interface em integração...</p>
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-md p-4 italic">
           <div className="bg-white w-full max-w-3xl rounded-[32px] overflow-hidden shadow-2xl">
              <div className="p-6 bg-black flex justify-between items-center">
                 ...
                 <button onClick={() => setSelected(null)} className="text-white font-black">X</button>
              </div>
              <div className="p-8">
