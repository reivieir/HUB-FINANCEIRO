...
const App: React.FC = () => {
  ...
  const handleAskAI = async (e: React.FormEvent | null) => {
    ...
    try {
      // FIX: Enviar apenas a string mata o erro "t is not iterable"
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
              ...
              {messages.map((msg, i) => (
                <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className="max-w-[80%] p-6 rounded-2xl bg-white border shadow-sm">
                    <p className="text-[10px] font-black uppercase opacity-30 mb-2">{msg.role === 'user' ? 'Você' : 'Assistente'}</p>
        ...
        <div className="p-8 bg-white border-t">
          <form onSubmit={handleAskAI} className="max-w-4xl mx-auto flex gap-4">
            <input className="flex-1 p-5 bg-gray-50 rounded-2xl border outline-none" placeholder="Digite aqui..." value={aiQuery} onChange={e => setAiQuery(e.target.value)} />
